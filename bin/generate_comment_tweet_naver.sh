...
TODAY="$(date '+%Y-%m-%d')"
###################################################################
TRANS_MODEL_DIR="../results/naver/saved_transformer_model"
TRANS_VOCAB_FILE="../results/naver/saved_transformer_model/vocab"
###################################################################
...
#print_if_err "generate nmt comment"
...
print_if_err "generate transformer comment"
echo "${TRANSFORMER_OUTPUT}"

# tweet generated comments
echo ""
echo "[$(date +"%T")][UPLOAD TWEET]"
NEWS_TF_TEXT="$(echo -e "${NEWS_TITLE}\n${NEWS_URL}\n\n${TRANSFORMER_OUTPUT}")"
./upload_tweet_naver.sh "${NEWS_TF_TEXT}"
print_if_err "upload tweet"
