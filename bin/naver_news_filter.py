...
def add_arguments(arg_parser):
    ...
    arg_parser.add_argument("--with-time", action="store_true", help="print also times comments have been written")
    return arg_parser

# 걸러진 댓글들을 반환하는 함수
def filter_comment(archive_file, max_count, with_time):
    ...
    result_dict = {"title":title, "comments":[], "time": []}
...
if __name__ == "__main__":
    ...
    result = [filter_comment(open(archive), args.max_comment_num, args.with_time) for archive in archives if filter_stupid_news(archive)]
    if args.out_plain_text:
        for r in result:
            if args.with_time:
                comments = [str(time) + " " + comment.replace("\n", " ") for (comment, time) in zip(r["comments"], r["time"])]
            else:
